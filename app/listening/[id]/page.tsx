...
import { Headphones, Clock, ChevronLeft, Home, Maximize2, Minimize2, AlertCircle } from 'lucide-react';
      ...
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="text-center">
          ...
          <Link href="/listening" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground">
            <ChevronLeft className="h-4 w-4" /> Back to Listening Tests
          </Link>
        </div>
      </div>
    );
          ...
          <div className="space-y-3 mb-6">
            <div className="rounded-lg border border-border p-4">
              <div className="flex items-center gap-2 mb-1">
                <span className="inline-flex items-center rounded-full bg-violet-500/10 px-2 py-0.5 text-xs font-semibold text-violet-600">IELTS Listening</span>
                <span className="text-xs text-muted-foreground">{test.category} · {test.difficulty}</span>
              </div>
              <p className="text-sm">This test contains 4 parts with 40 questions total. Audio plays once automatically.</p>
            </div>
          </div>

          <div className="flex items-start gap-2 rounded-lg bg-violet-500/10 border border-violet-200 p-3 mb-6">
            <AlertCircle className="h-4 w-4 text-violet-600 mt-0.5 flex-shrink-0" />
            <div className="text-xs text-violet-800">
              <p className="font-semibold mb-1">Important:</p>
              <ul className="list-disc list-inside space-y-0.5">
                <li>Audio plays automatically — listen carefully</li>
                <li>You can skip forward/backward within audio</li>
                <li>Answer all questions before time runs out</li>
                <li>Check answers at the end</li>
              </ul>
            </div>
          </div>

          <div className="flex gap-3">
            <Link href="/listening" className="flex-1 rounded-xl border border-border px-4 py-3 text-center text-sm font-medium hover:bg-muted transition-colors">
              Cancel
            </Link>
    ...
    <div className="flex h-screen flex-col bg-background">
      ...
      <header className="flex h-14 flex-shrink-0 items-center justify-between border-b border-border bg-card px-4">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <Link href="/listening" className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-sm font-medium text-muted-foreground hover:bg-muted flex-shrink-0">
            <ChevronLeft className="h-4 w-4" />
            <span className="hidden sm:inline">Listening</span>
          </Link>
          <div className="h-4 w-px bg-border flex-shrink-0" />
          <div className="flex items-center gap-1.5 rounded-lg bg-violet-500/10 px-2 py-1 min-w-0">
            <Headphones className="h-3 w-3 text-violet-600 flex-shrink-0" />
            <span className="text-xs font-semibold text-violet-600 truncate">{test.name}</span>
          </div>
        </div>
